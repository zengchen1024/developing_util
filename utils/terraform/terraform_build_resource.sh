...
sdk_dir=$(get_file_relative_path $2 $code_home_dir/vendor/)
api_doc_dir=$3

resource_name=$(echo $4 | awk -F ':' '{print $1}')
child_resource_name=$(echo $4 | awk -F ':' '{print $2}')

resource_go_file=$(pwd)/$5
...
generator=$tool_dir/new_python_tools/generate_resource_schema.py

err=$(python $generator $api_doc_dir $resource_go_file)
test $? -ne 0 && echo "generate resource schema failed: $err" && exit 1
