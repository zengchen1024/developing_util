...
src_base_dir="$($get_config using_sdk $src_cloud_alias)/"
src_relative_dir=${src#$src_base_dir}
dest=$($get_config using_sdk $dest_cloud_alias)/$src_relative_dir
if [ $# -eq 3 ]; then
	dest=$($get_config using_sdk $dest_cloud_alias)/$3
fi
